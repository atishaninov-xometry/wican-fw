...
- **Batch round-trip timing** — `/sdcard/obd_logs/batch_timing.csv` (`autopid_log_batch_timing()`, autopid.c), one row per batched Mode-01 request: `timestamp_utc,pids,round_trip_ms,success`. Written per-line (fopen/fprintf/fclose each time, no buffering to lose), so no safe-eject needed to read it. Added to measure the real OBD-gateway per-request latency floor for the 10-20Hz-on-a-few-PIDs question — pre-batching data implied ~770ms/PID average across a ~13-PID pass, which would put even a minimal RPM/Speed/Gear batch around 1.5-3Hz, not 10-20Hz; this log gives the actual number instead of that estimate.
