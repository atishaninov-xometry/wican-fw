...
const char device_config_default[] = "{\"wifi_mode\":\"AP\",\"ap_ch\":\"6\",\"webhook_en\":\"enable\",\"sta_ssid\":\"MeatPi\",\"sta_pass\":\"TomatoSauce\",\"sta_security\":\"wpa3\",\
								...
								\"batt_mqtt_pass\":\"meatpi\",\"batt_alert_time\":\"1\",\"mqtt_en\":\"disable\",\"mqtt_elm327_log\":\"disable\",\"elm327_udp_log\":\"disable\",\"mqtt_url\":\"mqtt://127.0.0.1\",\"mqtt_port\":\"1883\",\
										...
										\"logger_status\":\"disable\",\"log_filesystem\":\"littlefs\",\"log_storage\":\"sdcard\",\"log_period\":\"5\",\"log_poll_period\":\"10\"}";
...
//	cJSON_AddStringToObject(root, "can_datarate", device_config.can_datarate);
	...
	cJSON_AddStringToObject(root, "log_poll_period", device_config.log_poll_period);
...
static void config_server_load_cfg(char *cfg)
{
	...
	{
		strlcpy(device_config.log_period, "5", sizeof(device_config.log_period));
	}
	else
	{
	...
	if(key == 0 || key->valuestring == NULL)
	{
		strlcpy(device_config.log_poll_period, "10", sizeof(device_config.log_poll_period));
	}
	else
	{
		// milliseconds: 300000 (5min) ceiling matches the old 300s log_period range
		// this field used to share before it was split out.
		long log_poll_period = strtol(key->valuestring, NULL, 10);

		if(log_poll_period < 10 || log_poll_period > 300000)
		{
			log_poll_period = 10;
		}
	...
	ESP_LOGI(TAG, "device_config.log_poll_period: %s", device_config.log_poll_period);
	//*****
...
{
	char *endptr;
	long log_int = strtol(device_config.log_poll_period, &endptr, 10);

	if (*endptr != '\0' || endptr == device_config.log_poll_period)
	{
		return -1;
	}

	if (log_int < 10 || log_int > 300000)
	{
		return -1;
	}

	*log_poll_period = (uint32_t)log_int;
	return 1;
}
